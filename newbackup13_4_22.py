...
class Thread(QThread):
    ...
    def detect(self, frame):
        ...
        for (x, y, w, h) in fruitcascades:
            ...
            cv.cv2.putText(frame, 'fruits', ((x + w) - 10, y - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (30, 255, 30),
                           2)
        for (x, y, w, h) in applecascades:
            frame = cv.cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 2)
            cv.cv2.putText(frame, 'apple', ((x + w) - 30, (y + h) - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.9,
                           (30, 255, 30), 2)
        # for (x, y, w, h) in bananacascades:
        # frame = cv.cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 2)
    ...
    """
                self.blob = cv2.dnn.blobFromImage(cv_img, 1 / 255.0, (self.widthHeight, self.widthHeight), [0, 0, 0], 1,
                                                  crop=False)
...
three_ds_cascade = cv.cv2.CascadeClassifier(
    'updated_haar_images/classifier/cascade.xml')  # finds the classifier in the path
